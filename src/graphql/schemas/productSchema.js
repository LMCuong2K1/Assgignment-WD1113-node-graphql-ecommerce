...
const { createProductSchema, updateProductSchema } = require("../../utils/product.validation");
...
const getSelectString = (fieldsObj) => {
    return Object.keys(fieldsObj)
        .filter(key => typeof fieldsObj[key] === 'object' && Object.keys(fieldsObj[key]).length === 0 || key !== '__typename')
        .join(" ");
};
    ...
    resolvers: {
        Query: {
            products: async (_, args, context, info) => {
                ...
                const selectString = fieldsObj.products ? getSelectString(fieldsObj.products) : "";
